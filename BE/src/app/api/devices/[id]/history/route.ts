...
// GET: time-series history teragregasi (bucket cellMin/Max/Avg/delta, suhu, arus/daya, energi
// in/out) buat envelope chart FE. Query: ?hours=24&bucket=auto|<detik>&cells=0|1
export const GET = route<{ params: Promise<{ id: string }> }>(async (req, { params }) => {
  const session = await requireAuth();
  const { id } = await params;

  // Otorisasi terpusat: owner ATAU collaborator, sama seperti GET /api/devices dan endpoint
  // v1 mobile. Melempar 404/403 terformat lewat http.ts (ditangkap oleh wrapper route()).
