...
## 13. Fleet Dashboard polish (branch `fix/fleet-polish`, dari `main` setelah §12 live)

Perbaikan temuan produksi: 4 bug + 3 polish. FE-only (tidak ada perubahan BE di putaran ini).

### 13a. Bug

| # | Temuan | Hasil investigasi | Tindakan |
|---|---|---|---|
| 1 | Sidebar: background navy berhenti di ~928px pada halaman yang lebih tinggi dari viewport | **Tidak reproduksi** — `<aside>` sudah `fixed h-screen overflow-y-auto` (bukan `sticky`, tapi secara fungsional setara: viewport-relative, top-pinned). Diverifikasi via `getComputedStyle`/`getBoundingClientRect` di dev **dan** production build (`next build` standalone, `next start`-equivalent) pada Dashboard (page ~1584px) dan Device Detail (page ~3371px) di 1920×1080 **dan** 1366×768 — `aside.height` selalu persis = `window.innerHeight`, tidak ada ancestor ber-`transform` yang bisa merusak containing block `fixed`. Satu petunjuk dekat: resize viewport TANPA reload (lewat tool testing) sempat menyisakan `aside` di tinggi lama sampai reload — tapi ini device-metric-override quirk alat testing, bukan bug CSS/React (browser asli me-recompute `100vh` terus-menerus tanpa perlu reload). **Hardening defensif** tetap diterapkan: `html, body { min-h-screen }` (sebelumnya cuma wrapper dalam `AdminLayout`). Bila masih terlihat di produksi setelah deploy ini, mohon screenshot + info browser/OS untuk investigasi lanjut — restrukturisasi ke `sticky`+flex (opsi B di brief) sengaja TIDAK dilakukan karena berisiko tinggi terhadap perilaku expand/hover/mobile-overlay sidebar di seluruh app, untuk bug yang belum bisa dikonfirmasi ulang. |
| 2 | `DeviceCard` footer: device stale/offline menampilkan "OK" | Badge status tidak mengikuti freshness | Footer sekarang: **live** → status alarm (`OK` hijau / `N Warn` amber jika ada alarm severity warning / `N Critical` merah jika ada severity critical); **stale/offline** → label netral `Data basi · <umur>` (abu-abu, tanpa kata "OK") |
| 3 | "Mendekati ambang" di Needs Attention terlalu sensitif (17 mV vs warn 20 mV) | Floor lama = 70%×warn (14 mV) | Floor baru = `imbalanceNearingCriticalRatio` (0.8) × `imbalanceCriticalMv` (50) = **40 mV**, konstanta di `ALERT_THRESHOLDS` (`alertRules.ts`). Devices dengan delta di [20,40) mV (warning-severity, belum "nearing critical") tetap kategori "alarm" biasa — konsisten dengan badge "Warn" baru di DeviceCard; hanya [40,50) mV yang dapat label spesifik "nearing" |
| 4 | Verifikasi konvensi tanda arus end-to-end | **Konsisten, tidak ada yang terbalik.** Ground truth (simulator `tools/simulator/index.mjs`): `power = packVoltage × current`, `current` negatif saat fase "charge" → power negatif = charging. BE `schema.ts`/`ingest.ts`: diteruskan apa adanya, tanpa transformasi. FE: `packMetrics.currentDirection` (`current<0→charging`) dipakai `BatteryTwin`/`PackCard`; `DeviceCard`/`fleetKpi` pakai aturan yang sama di atas `power` (bukan `currentDirection`, karena beroperasi di level Watt agregat, bukan Ampere per-pack — didokumentasikan di komentar kode); `energyInWh`/`energyOutWh` (BE `history/route.ts` & `energy.ts`): kontribusi negatif (power negatif = charging) → `energyInWh`. Semua konsisten "negatif = charging = masuk". |

### 13b. Polish

- **FleetPulse hero** dirombak jadi visual utama: SVG lebih besar (`viewBox 0 0 640 170`), tiga node **Sumber → Baterai (SoC agregat live, fill proporsional) → Beban**, dua segmen wire independen (Sumber→Baterai untuk `chargeW`, Baterai→Beban untuk `dischargeW`) — partikel jalan hanya di segmen yang aktif, jumlah (1–5) & kecepatan (0.7–1.8s/siklus) proporsional terhadap besar daya segmen itu (ilustratif, bukan kalibrasi fisik presisi). Reuse kelas `.twin-flow`/`--twin-flow-dx` (BatteryTwin, `globals.css`) — durasi di-override per instance via inline style, **tanpa keyframe baru**. `useReducedMotion()` tetap dihormati (partikel di-skip total). Angka charge/discharge/net tetap ada, dipindah ke bawah visual.
- **HealthDistribution**: kolom label `w-28` → `sm:w-40` (dari `w-24`) + `title` atribut nama lengkap — nama device tidak lagi kepotong untuk nama wajar; nama sangat panjang tetap `truncate` dengan tooltip sebagai fallback.
- **LiveEventFeed**: "Packet masuk" tidak lagi satu baris per paket — sekarang satu baris per device yang diperbarui di tempat (`Packet masuk ×N · terakhir Ns lalu`), disimpan terpisah dari buffer 50 event (transisi freshness + alarm). Event transisi/alarm dirender lebih menonjol (border + tint warna per kategori) di atas baris packet yang lebih kecil/muted di bawahnya.

### 13c. Verifikasi

- `npx tsc --noEmit` bersih, `next build` bersih (Node 22). `eslint`: nol error/warning baru (2 error `react-hooks/set-state-in-effect`/`purity` yang tersisa tetap pre-existing di `main`, tidak disentuh).
- BE tidak diubah di putaran ini — suite `test:db` yang sudah ada tidak perlu dijalankan ulang (tidak ada risiko regresi BE).
- Diuji hidup di stack lokal yang sama (simulator 3 device): FleetPulse hero baru terkonfirmasi (Sumber/Baterai 73–76% SoC/Beban dengan partikel dua arah), Needs Attention tidak lagi menandai delta 10–16 mV, DeviceCard footer "OK" untuk live tanpa alarm, LiveEventFeed menampilkan baris "Packet masuk ×1 · terakhir Ns lalu" terpisah dari baris alarm/transisi bertanda (`Alarm reda`, `delta 84 mV > 50 mV`) berwarna. Light, dark, dan 390px diverifikasi di Dashboard; sidebar full-height diverifikasi di Dashboard dan Device Detail pada 1920×1080 dan 1366×768.
